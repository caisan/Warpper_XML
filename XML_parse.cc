...
static void xml_start(void *data, const char *el, const char **attr) {
   ...
   XMLParser *handler = static_cast<XMLParser *>(data);
...
XMLParser::
XMLParser() : buf(NULL), buf_len(0), cur_obj(NULL), success(true)
{
...
bool XMLParser::xml_start(const char *el, const char **attr) {
...
bool XMLParser::xml_end(const char *el) {
...
void XMLParser::handle_data(const char *s, int len)
{
...
bool XMLParser::parse(const char *_buf, int len, int done)
{
    int pos = buf_len;
    char *tmp_buf;
